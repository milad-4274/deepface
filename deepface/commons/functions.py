...
sys.path.insert(1, dir_path+'/RetinaFaceAntiCov')
...
def initialize_detector(detector_backend):
	...
	if detector_backend == 'opencv':
		...
		# face_detector = MTCNN()
		face_detector = RetinaFaceCoV(
			'deepface/commons/RetinaFaceAntiCov/model/mnet_cov2', 0, 0, 'net3l')
...
def detect_face(img, detector_backend = 'opencv', grayscale = False, enforce_detection = True):
	...
	if detector_backend == 'opencv':
		...
		return get_retina(img, enforce_detection,'face')
...
def align_face(img, detector_backend = 'opencv'):
	...
	if (detector_backend == 'opencv') or (detector_backend == 'ssd'):
		...
		landmark = get_retina(img,out='landmark')
		if landmark[0]:
			landmark = landmark[1]
...
def get_retina(img,enforce_detection=False,out='face'):
